                        ...
                        <div class="border-2 border-dashed border-gray-300 rounded-xl p-8 text-center hover:border-purple-400 transition-colors" id="dropZone">
                            <div id="uploadArea">
                                ...
                                <label for="foto" class="bg-purple-600 hover:bg-purple-700 text-white px-6 py-3 rounded-lg cursor-pointer transition-colors inline-flex items-center space-x-2">
                                    ...
                                    <span>Pilih Gambar dari Folder</span>
                                </label>
                                <input type="file" name="foto" id="foto" class="hidden" accept="image/jpeg,image/jpg,image/png,image/gif,image/webp">
                                <p class="text-xs text-gray-400 mt-3">JPG, PNG, GIF, WEBP (Max: 5MB)</p>
                            </div>
...
document.addEventListener('paste', function(e) {
    ...
    for (let i = 0; i < items.length; i++) {
        if (items[i].type.indexOf('image') !== -1) {
            ...
            alert('✅ Gambar dari clipboard berhasil ditempelkan!');
            e.preventDefault();
            break;
        }
    }
...
function updatePreview() {
    ...
    if (previewImg.src && !imagePreview.classList.contains('hidden')) {
        imageHtml = `<img src="${previewImg.src}" class="w-full h-32 object-cover rounded-lg mb-3" alt="Cover">`;
    }
    
    if (title || content || imageHtml) {
        preview.innerHTML = `
            <div class="space-y-3">
                ${imageHtml}
